...
# The hcloud-csi chart reads the namespace-local `hcloud` Secret created
# above. The CCM uses a separate kube-system Secret with the same token.
# ---------------------------------------------------------------------------
...
resource "kubectl_manifest" "argo_app_hcloud_csi" {
  ...
  depends_on = [terraform_data.argocd_gate]
}
...
# ---------------------------------------------------------------------------
# Readiness gate. Downstream layers (platform) need both the StorageClass
# and the CNPG controller before they can apply Cluster CRs. Same pattern as
# platform's cert-manager wait.
#
