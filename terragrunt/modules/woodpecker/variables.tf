...
variable "platform_data_ready" {
  description = "Sentinel from 35-platform-data. Forces this layer to wait until the StorageClass is present."
  type        = bool
}

variable "forgejo_url" {
  description = "Public Forgejo URL. From 55-forgejo output."
  type        = string
}

variable "forgejo_namespace" {
  description = "Namespace where Forgejo is installed. From 55-forgejo output."
  type        = string
}

variable "forgejo_ready" {
  description = "Sentinel from 55-forgejo. Forces this layer to wait until Forgejo is healthy before creating the OAuth app."
  type        = bool
}
