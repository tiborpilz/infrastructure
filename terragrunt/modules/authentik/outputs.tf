...
output "bootstrap_admin_token" {
  description = "Initial akadmin API token. Consumed by 45-authentik-config to configure authentik via the goauthentik/authentik provider."
  value       = random_password.bootstrap_admin_token.result
  sensitive   = true
}

output "bootstrap_admin_password" {
  description = "Initial akadmin password. Replaced by a stable TF-pinned password in 45-authentik-config; this output exists only so the operator can log in before that layer applies."
  value       = random_password.bootstrap_admin_password.result
  sensitive   = true
}

output "ready" {
  description = "Sentinel that 45-authentik-config depends on to ensure authentik is reachable before talking to its API."
