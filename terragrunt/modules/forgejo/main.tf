locals {
  ...
  # `var.platform_data_ready` and `var.authentik_ready` are consumed by
  # Terragrunt's dependency graph. Declared but not otherwise referenced by
  # design: ordering is enforced at the layer boundary.
}
