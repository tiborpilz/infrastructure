...
variable "storage_class" {
  description = "StorageClass name for Forgejo data and CNPG PVCs. From 35-platform-data output."
  type        = string
  default     = "hcloud-volumes"
}

variable "platform_data_ready" {
  description = "Sentinel from 35-platform-data. Forces this layer to wait until the StorageClass + CNPG operator are present."
  type        = bool
}
