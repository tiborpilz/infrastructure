  ...
  config_patches = [
    ...
    yamlencode({
      machine = {
        ...
        # Lets a pod in the etcd-backup namespace call the Talos API for etcd
        # snapshots, scoped to os:etcd:backup. Consumed by the talos-backup
        # CronJob (applications/operators/etcd-backup); applied in place.
        features = {
